...
// Bandra Center
const BANDRA_CENTER = { lat: 19.0596, lng: 72.8295 };
...
const seedData = async () => {
    try {
        ...
        console.log("Connected to MongoDB for seeding Bandra...");
        ...
        for (let i = 0; i < 10; i++) {
            ...
            const lat = BANDRA_CENTER.lat + latOffset;
            const lng = BANDRA_CENTER.lng + lngOffset;
            ...
            const newIssue = {
                _id: issueId,
                title: `${categoryObj.label.charAt(0).toUpperCase() + categoryObj.label.slice(1)} in Bandra ${i + 1}`,
                ...
                defaultDescription: `Test report for ${categoryObj.label} in the Bandra West area.`,
            ...
            const newReport = {
                ...
                user_id: user._id,
                imageUrl: imageUrl,
                description: `Detailed report for ${categoryObj.label} in Bandra.`
            };
        ...
        console.log(`Successfully seeded 10 issues and 10 reports in Bandra area.`);
        process.exit(0);
    } catch (error) {
        console.error("Error seeding Bandra data:", error);
        process.exit(1);
    }
};
